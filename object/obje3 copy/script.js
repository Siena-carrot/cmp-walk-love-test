...
// obje3は「正解済み」なら正常表示、それ以外は異常表示// obje2は「正解済み」なら正常表示、それ以外は異常表示
...
function confirmChoice(choice) {
  const lockUntilLocal = parseInt(localStorage.getItem("obje3LockUntil"), 10);
...
function showMap() {
  ...
  function showMap(button) {
    try {
      ...
      // Only display the per-map description if provided. Do not fallback to
      // the object's general description — leave blank otherwise.
      let desc = '';
      let mapDesc = null;
      if (section) {
        mapDesc = section.querySelector('.map-description');
