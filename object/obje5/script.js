const correctChoice = "c3";
const userAnswer = localStorage.getItem("obje5Answer");
const loggedIn = localStorage.getItem("adminLoggedIn");
const lockUntil = parseInt(localStorage.getItem("obje5LockUntil"), 10);
const now = Date.now();
...
function checkAnswer(choice) {
  if (choice === correctChoice) {
    localStorage.setItem("obje5Answer", "restored");
    alert("ページの復旧が確認できました");
    location.reload();
  } else {
    const lockTime = Date.now() + 15 * 60 * 1000;
    localStorage.setItem("obje5LockUntil", lockTime);
...
function showMap(button) {
  try {
    ...
    // Use only .map-description when present; otherwise leave description empty
    let desc = '';
    if (section) {
    ...
    if (!filename) filename = 'obje5-map.jpg';
